# Catalyst Templates

Welcome to the Catalyst Game Panel template repository! This repository contains organized templates for game servers, databases, and various applications that can be deployed through the Catalyst control panel.

## 📁 Repository Structure

### 🎮 Games
The `games/` directory contains templates for various game servers, organized by genre:

- **`games/action/`** - Action game servers (e.g., GTA V, Left 4 Dead 2, etc.)
- **`games/adventure/`** - Adventure game servers (e.g., Valheim, Don't Starve Together, etc.)
- **`games/fps/`** - First-Person Shooter game servers (e.g., CS:GO, CS2, Squad, etc.)
- **`games/mmo/`** - Massively Multiplayer Online game servers (e.g., World of Warcraft, etc.)
- **`games/moba/`** - Multiplayer Online Battle Arena servers (e.g., Dota 2, etc.)
- **`games/racing/`** - Racing game servers
- **`games/rts/`** - Real-Time Strategy game servers
- **`games/sandbox/`** - Sandbox game servers (e.g., Minecraft, Terraria, etc.)
- **`games/simulation/`** - Simulation game servers
- **`games/sports/`** - Sports game servers
- **`games/strategy/`** - Strategy game servers
- **`games/survival/`** - Survival game servers (e.g., Rust, ARK, etc.)

### 🗄️ Databases
The `databases/` directory contains templates for database servers:
- MySQL, MariaDB, PostgreSQL, MongoDB, Redis, etc.

### 🎙️ Voice Servers
The `voice-servers/` directory contains templates for voice communication servers:
- TeamSpeak, Mumble, Discord bots with voice capabilities, etc.

### 🌐 Web Servers
The `web-servers/` directory contains templates for web server applications:
- Apache, Nginx, Node.js applications, etc.

Available templates:

| Template | Path | Description |
|----------|------|-------------|
| FastDL (Caddy) | [`web-servers/fastdl/`](web-servers/fastdl/) | Standalone `sv_downloadurl` content server for HL1/Source games (CS 1.6, GMod, CS:S, TF2, …) |

### 💾 Storage
The `storage/` directory contains templates for storage solutions:
- File servers, FTP servers, MinIO, etc.

### 🤖 Bots
The `bots/` directory contains templates for various bot applications:
- Discord bots, Telegram bots, IRC bots, game bots, etc.

### 📦 Miscellaneous
The `misc/` directory contains templates for other applications that don't fit into the above categories.

## 🚀 How to Use

Each template directory should contain:
1. A configuration file defining the server setup
2. A README with specific instructions
3. Any required startup scripts or configurations

## 🤝 Contributing

When adding a new template, please:
1. Place it in the appropriate category folder
2. Include a detailed README.md
3. Follow the existing template structure
4. Test your template before submitting

## 📝 Template Format

Templates should follow the Catalyst panel specification and include:
- Server startup command
- Configuration file locations
- Port requirements
- Resource requirements (CPU, RAM, disk)
- Installation steps

## 📄 License

See the [LICENSE](LICENSE) file for details.